...
<script>
    ...
    window.errors = {
        !!json_encode($errors - > getMessages() ?? []) !!
    };
    ...
    $(document).ready(function() {
        ...
        function renderFieldGroup(label, fieldClass, fieldName, inputType = 'text', colClass = 'col-md-6') {
            ...
            if (inputType === 'textarea') {
                html +=
                    `<textarea class="form-control ${fieldClass}" rows="3" data-field="${fieldName}"></textarea>`;
            } else {
                const step = inputType === 'number' ? '0.01' : 'any';
                const accept = inputType === 'file' ? 'accept="image/*"' : '';
                html +=
                    `<input type="${inputType}" step="${step}" class="form-control ${fieldClass}" value="" data-field="${fieldName}" ${accept}>`;
            }
        ...
        function renderTable() {
            ...
            window.variantsStore.forEach((variant, idx) => {
                const $card = $('<div>').addClass(
                    'card mb-3 border-start border-start-3 border-primary');
                ...
                Object.entries(fieldMap).forEach(([className, fieldName]) => {
                    ...
                    if ($input.length) {
                        $input.val(variant[fieldName] || '').data('idx', idx).on('change',
                            updateVariant);
                ...
                $header.on('click', function() {
                    ...
                    $body.toggle();
                    $(this).find('i').css('transform', isVisible ? 'rotate(0deg)' :
                        'rotate(180deg)');
                });
            });

            // Attach event listeners
            $variantsList.on('change', '.variant-exchangeable, .variant-refundable, .variant-free-shipping',
                updateVariantCheckbox);
        ...
        function updateVariantCheckbox(e) {
            ...
            else if ($this.hasClass('variant-free-shipping')) variant.free_shipping = $this.is(':checked') ? 1 :
                0;
        }
        ...
        if ($productTypeSelect.length) {
            function toggleVariantsSection() {
                ...
                if (isVariants && window.variantsStore && window.variantsStore.length > 0 && $variantsList.is(
                        ':empty')) {
                    renderTable();
                }
            }
