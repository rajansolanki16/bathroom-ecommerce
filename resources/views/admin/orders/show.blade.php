    ...
    <div class="card">
        <div class="card-header d-flex align-items-center justify-content-between flex-nowrap">
            ...
            <div class="d-flex gap-2 align-items-center ms-3">
                ...
                <span class="badge bg-info">Showing: {{ $orders->firstItem() ?? 0 }} -
                    {{ $orders->lastItem() ?? ($orders->count() ?? 0) }}</span>
            </div>
        </div>
        <div class="card-body">
            ...
            <div class="btn-group">
                ...
                <ul class="dropdown-menu">
                    <li>
                        <a class="dropdown-item" data-export="csv"
                            href="{{ route('orders.export', 'csv') }}?{{ http_build_query(request()->query()) }}">
                            <i class="bi bi-filetype-csv me-2"></i>Export CSV (filtered)
                        </a>
                    </li>
                    <li>
                        <a class="dropdown-item" data-export="excel"
                            href="{{ route('orders.export', 'excel') }}?{{ http_build_query(request()->query()) }}">
                        ...
                        <div class="col-xxl">
                            <div class="search-box">
                                <input type="text" name="search" value="{{ request('search') }}"
                                    class="form-control" placeholder="Search products and User...">
                            ...
                            <tr>
                                ...
                                <td>
                                    @if ($order->items->count())
                                        @foreach ($order->items as $item)
                                            <div><strong>{{ $item->product?->product_title ?? 'N/A' }}</strong> <small
                                                    class="text-muted">x{{ $item->quantity ?? 1 }}</small></div>
                                        @endforeach
                                    @else
                                        N/A
                                    @endif
                                ...
                                <td>
                                    <div class="d-flex align-items-center gap-2">
                                        ...
                                        <span
                                            class="badge bg-secondary status-label-{{ $order->id }}">{{ optional($order->status)->label() }}</span>
                                        <button type="button" class="btn btn-outline-secondary btn-sm edit-notes-btn"
                                            data-id="{{ $order->id }}" data-notes="{{ e($order->internal_notes) }}"
                                            title="Edit internal notes">
                                            <i
                                                class="bi bi-chat-left-text{{ $order->internal_notes ? '-fill text-primary' : '' }}"></i>
                                        </button>
                                    </div>
                                </td>
...
<script>
    $(function() {
                    ...
                    success: function(data) {
                        if (data.success) {
                            ...
                            if (typeof showToast === 'function') showToast(
                                'Status updated to: ' + data.status, 'success');
                        } else {
                            if (typeof showToast === 'function') showToast(data.message ||
                                'Failed to update status', 'danger');
                        }
                    },
                    error: function(xhr) {
        ...
        function bindNotesHandlers() {
            ...
            $(document).off('click.editNotes', '.edit-notes-btn').on('click.editNotes', '.edit-notes-btn',
                function() {
                    var id = $(this).data('id');
                    ...
                    if (!modalEl) {
                        ...
                        if (typeof showToast === 'function') showToast(
                            'Notes modal not found (contact admin)', 'danger');
                        return;
                    }
                    if (typeof bootstrap === 'undefined' || !bootstrap.Modal) {
                        console.error('Bootstrap JS not loaded or bootstrap.Modal undefined');
                        if (typeof showToast === 'function') showToast(
                            'Bootstrap JS not loaded (modal unavailable)', 'danger');
                        return;
                    }
        ...
        $('#saveNotesBtn').off('click.saveNotes').on('click.saveNotes', function() {
            ...
            console.log('Saving notes for order', id, 'payload:', {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.success) {
                        ...
                        $btn.data('notes', data.internal_notes).attr('data-notes', data
                            .internal_notes);
                        var $icon = $btn.find('i');
                        if (data.internal_notes) {
                            $icon.removeClass('bi-chat-left-text').addClass(
                                'bi-chat-left-text-fill text-primary');
                        } else {
                            $icon.removeClass('bi-chat-left-text-fill text-primary')
                                .addClass('bi-chat-left-text');
                        }
                        var modalEl = document.getElementById('notesModal');
                        if (!modalEl) {
                            ...
                            console.warn(
                                'bootstrap.Modal not available; cannot hide modal programmatically'
                                );
                        } else {
